...
BoxShader::BoxShader(ID3D11Device* device, HWND hwnd) : BaseShader(device, hwnd)
{
...
void BoxShader::InitShader(WCHAR* vsFilename, WCHAR* psFilename)
{
...
void BoxShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX &worldMatrix, const XMMATRIX &viewMatrix, const XMMATRIX &projectionMatrix, ID3D11ShaderResourceView* texture, int width, int height)
{
	HRESULT result;
...
void BoxShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
