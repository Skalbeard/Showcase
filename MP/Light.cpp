...
void Light::GenerateOrthoMatrix(float width, float screenNear, float screenFar)
{
	// Create ortho matrix for the light.
	m_orthoMatrix = XMMatrixOrthographicLH(width, width, screenNear, screenFar);
}
...
{
	XMFLOAT4 temp(XMVectorGetX(m_position), XMVectorGetY(m_position), XMVectorGetZ(m_position), XMVectorGetW(m_position));
	return temp;
}
...
void Light::SetUp(float x, float y, float z)
{
